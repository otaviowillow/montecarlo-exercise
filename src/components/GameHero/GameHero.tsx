...
const GameHero = (): JSX.Element | null => {
  const { id } = useParams();
  ...
  return (
    <Card>
      ...
      <Container>
        {RawgGame.developers?.map((dev, i) => <Typography key={i} variant="h4">{dev.name}</Typography>)}
        <p dangerouslySetInnerHTML={{ __html: sanitize(RawgGame.description || "") }} />
      </Container>
    </Card>
  )
}
